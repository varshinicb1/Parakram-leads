    ...
    <div className="min-h-screen pt-24 pb-32">
      <div className="max-w-7xl mx-auto px-6">
        <div className="mb-16">
          ...
          <p className="text-[15px] text-[#10b981] max-w-lg leading-relaxed">Every project is delivered on time, on spec, with obsessive attention to quality. For pricing, reach us on WhatsApp.</p>
        </div>
          ...
          {fullServices.map(({ name, icon: Icon, desc, time, tech }, i) => (
            <motion.div key={name} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }} className="relative p-7 group" style={{ border: "1px solid rgba(201,169,110,0.1)", background: "#0a0a0a" }} whileHover={{ borderColor: "rgba(201,169,110,0.3)", y: -2 }}>
              ...
              <div className="flex items-center justify-between border-t border-white/[0.05] pt-4 mb-3">
                <div><p className="text-[9px] font-mono text-[#10b981] uppercase mb-1">Typical Timeline</p><p className="text-[11px] font-mono text-[#10b981]">{time}</p></div>
                <div className="text-right"><p className="text-[9px] font-mono text-[#10b981] uppercase mb-1">Pricing</p><a href="https://wa.me/919901823011" target="_blank" rel="noopener noreferrer" className="text-[11px] font-mono text-[#25D366]/70 hover:text-[#25D366] transition-colors">WhatsApp us</a></div>
              </div>
              <div className="flex items-center gap-2 flex-wrap mb-4">{tech.map(t => <span key={t} className="text-[9px] font-mono px-2 py-0.5 text-[#10b981] border border-white/[0.05]">{t}</span>)}</div>
