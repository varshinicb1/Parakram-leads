...
import { GoldChest } from "../components/GoldChest";
import { GoldHunt } from "../components/GoldHunt";
import { GoldParticles } from "../components/GoldParticles";
import { addCoins } from "../components/GameHUD";
import {
  ArrowRight, ChevronRight, ExternalLink,
  Globe, Star, Smartphone, Bot, Cpu, Microscope,
  BarChart3, MessageCircle, Github, Linkedin, Gamepad2,
} from "lucide-react";
    ...
    <div className="min-h-screen">
      ...
      <SectionFade>
        <section className="max-w-7xl mx-auto px-6 py-24">
          <div className="flex items-end justify-between mb-12">
            ...
            <button onClick={() => go("services")} className="hidden md:flex items-center gap-2 text-[12px] font-mono text-[#10b981] hover:text-[#c9a96e] transition-colors">All Services <ArrowRight size={12} /></button>
          </div>
              ...
              <StaggerItem key={name}>
                <motion.div onClick={() => go("services")} className="relative p-6 cursor-pointer group" style={{ border: "1px solid rgba(201,169,110,0.1)", background: "#0a0a0a" }} whileHover={{ y: -4, borderColor: "rgba(201,169,110,0.35)" }} transition={{ duration: 0.2 }}>
                  ...
                  <div className="flex items-start justify-between mb-5">
                    <div className="w-9 h-9 flex items-center justify-center border border-white/[0.07] group-hover:border-[#c9a96e]/30 transition-colors bg-white/[0.02]"><Icon size={16} className="text-[#10b981] group-hover:text-[#c9a96e] transition-colors" /></div>
                    <span className="text-[9px] font-mono text-[#10b981] group-hover:text-[#c9a96e]/50">{diff}</span>
                  </div>
                  <h3 className="text-[14px] font-semibold text-[#c8c6c3] mb-2 group-hover:text-[#e8e6e3] transition-colors">{name}</h3>
                  <p className="text-[12px] text-[#10b981] leading-relaxed mb-5">{desc}</p>
                  <div className="flex items-center gap-1 text-[11px] font-mono text-[#10b981] group-hover:text-[#c9a96e] transition-colors">Get Started <ChevronRight size={10} /></div>
                </motion.div>
              </StaggerItem>
            ))}
          </StaggerChildren>
          ...
          >
            <p className="text-[12px] text-[#10b981] mb-2">Need something not listed?</p>
                ...
                <StaggerItem key={domain}>
                  <motion.a href={`https://${domain}`} target="_blank" rel="noopener noreferrer"
                    ...
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <ExternalLink size={11} className="text-[#10b981] group-hover:text-[#c9a96e] transition-colors" />
                    </div>
                    ...
                    <p className="text-[12px] text-[#10b981] leading-relaxed">{desc}</p>
                  </motion.a>
                </StaggerItem>
              ))}
            </StaggerChildren>
      ...
      <SectionFade>
        <section className="max-w-7xl mx-auto px-6 py-24">
          ...
          <p className="text-[14px] text-[#10b981] mb-10 max-w-xl leading-relaxed">Beyond client work, we build independent products. <a href="https://leads.getparakram.in" target="_blank" rel="noopener noreferrer" className="text-[#c9a96e] hover:underline">Try Parakram Leads</a> — our AI-powered lead intelligence platform.</p>
          <StaggerChildren className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8" staggerDelay={0.08}>
            {[
              ...
              <StaggerItem key={name}>
                <Panel className="p-6" onClick={() => go("products")}>
                  ...
                  <p className="text-[10px] font-mono mt-2 text-[#10b981]">{pct}% complete</p>
                </Panel>
              </StaggerItem>
            ))}
          </StaggerChildren>
          ...
          <div className="max-w-7xl mx-auto px-6">
            <div className="grid md:grid-cols-2 gap-12 items-center">
              ...
              >
                ...
                <p className="text-[14px] text-[#10b981] leading-relaxed mb-8 max-w-md">Parakram is founded by Varshini CB — developer, designer, and systems thinker. Every product carries the same obsessive attention to craft.</p>
                ...
                <Panel title="character.profile" className="p-8" onClick={() => go("about")}>
                  <div className="flex items-start gap-5">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-[#10b981]">Full-Stack Developer & Systems Architect</p>
                      <div className="flex items-center gap-3 mt-3">
                        <a href="https://www.linkedin.com/in/varshini-cb-821176360/" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-[#10b981] hover:text-[#c9a96e] transition-colors"><Linkedin size={14} /></a>
                        <a href="https://github.com/varshinicb1" target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-[#10b981] hover:text-[#c9a96e] transition-colors"><Github size={14} /></a>
                      </div>
                    </div>
                  </div>
                  <div className="mt-5 pt-4 border-t border-white/[0.05]">
                    {[["DEV", 98], ["DESIGN", 92], ["AI / ML", 88], ["SYSTEMS", 95]].map(([k, v]) => (
                      <div key={String(k)} className="flex items-center gap-3 mb-2">
                        <span className="text-[9px] font-mono text-[#10b981] w-16">{k}</span>
      ...
      <SectionFade>
        <section className="max-w-7xl mx-auto px-6 py-24">
          <Panel className="p-10 text-center relative overflow-hidden">
            <div className="absolute inset-0 bg-[radial-gradient(ellipse_60%_70%_at_50%_50%,rgba(201,169,110,0.04),transparent_70%)] animate-glow-pulse" />
            <div className="relative z-10">
              <div className="flex items-center justify-center gap-3 mb-4">
                <GoldChest size={36} />
                <SectionLabel>Gold Hunt</SectionLabel>
                <GoldChest size={36} />
              </div>
              <h2 className="text-[28px] md:text-[38px] font-semibold text-[#e8e6e3] tracking-[-0.02em] mb-3" style={{ fontFamily: "Sora, sans-serif" }}>The Treasure Awaits.</h2>
              <p className="text-[13px] text-[#10b981] mb-8 max-w-lg mx-auto leading-relaxed">
                Navigate the dark maze, follow the detector, and claim the gold. Arrow keys to move, click to pathfind.
              </p>
              <div className="flex justify-center">
                <GoldHunt onTreasureFound={() => {
                  addCoins(10);
                  confetti({ particleCount: 60, spread: 70, origin: { y: 0.4 }, colors: ["#c9a96e", "#f5e4a8", "#10b981"] });
                }} />
              </div>
            </div>
          </Panel>
        ...
        <section className="max-w-7xl mx-auto px-6 py-24">
          <Panel className="p-16 text-center relative overflow-hidden">
            ...
            <div className="relative z-10">
              ...
              <p className="text-[14px] text-[#10b981] mb-10 max-w-md mx-auto leading-relaxed">Tell us what you want to build. We will figure out the rest together — from concept to shipped product.</p>
