...
$svcName = "JalebiVPS"
$svcDisplay = "JALEBI VPS Dashboard"
$svcDesc = "JALEBI VPS — Windows VPS management dashboard with real-time system monitoring, service control, and Cloudflare tunnel management."
...
switch ($Action) {
  "install" {
    ...
    # Firewall
    netsh.exe advfirewall firewall add rule name="JALEBI VPS Dashboard" dir=in action=allow protocol=TCP localport=9876 profile=any | Out-Null
  ...
  "uninstall" {
    ...
    netsh.exe advfirewall firewall delete rule name="JALEBI VPS Dashboard" 2>$null | Out-Null
